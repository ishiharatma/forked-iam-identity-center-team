...
if [ -z "$COGNITO_IDP_JSON" ]; then
  echo "COGNITO_IDP_JSON is not set."
  exit 1
fi
...
aws cognito-idp create-identity-provider --region $REGION --user-pool-id $cognitoUserpoolId --provider-name=IDC --provider-type SAML --provider-details file://${COGNITO_IDP_JSON} --attribute-mapping email=Email --idp-identifiers team
